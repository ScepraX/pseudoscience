...
| 1 | Bloot | N | honest / exposed | The state of having no Masker. The true Eigen presented without alteration. |
...
| 2 | Masker | M | mask / disguise | A presented Eigen (m) differing from the Bloot one, broadcast at the source. The partner's Echo faithfully tracks the mask; perceived binding may cross Drempel while the true binding stays below — a bond that appears bound on one side and is not. It costs Pijn (m): the Energie of continuous plausible updating while the true Eigen stays put. No Vonk fires for a crossing the true Eigen never made, so a Masker accumulates strain but no Greep, no Diepte, no Trouw. Opposite of Bloot (N). |
...
| 8 | Octaaf | ω | octave / wrapping | A rounded whole, the cycle completing. The wrapping of the prime ladder after 149; scale invariance across levels. Its spine is the eight-step walk: God (😇) at position 1, Godin (😳) at position 8, seven steps between — as Hydrogen to Oxygen in the Atom Octaaf. Their bond becomes the God of the next Octaaf.  |
