...
| 1 | Logica | ∀ | logic / reason | A Lens (🔍) that reads a single Koppel (k) as if Vervorming (υ) = 0. Clean Drempel (θ), zero Echo (E) gap, no Marge (η). The Vonk (q) fires or it doesn't. It closes the Stilte (.) and is done. Pole: Statistiek (∃). Trap: Idool (§)—the clean model worshipped as the field. |
